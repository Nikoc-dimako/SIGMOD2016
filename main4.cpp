...
int main() {
	...
	if(star){
		...
		// Main Thread Queue
		priority_queue<SP_Params, std::vector<SP_Params>, Mycomparison> main_queue;
		//main_queue.reserve(20000);

		priority_queue<SP_Params, std::vector<SP_Params>, Mycomparison> thread_queue;
		//thread_queue.reserve(20000);
		SP_Params thrQ,mainQ;
		...
		while(cin >> c) {
			if(c == 'F') {
				...
				while(!thread_queue.empty()){
					thrQ=thread_queue.top();
					query_pool.postWork<void>([thrQ] {  shortest_path_star(thrQ.a, thrQ.b, thrQ.versionCounter, thrQ.resultsCounter);  });
					thread_queue.pop();
				}
				...
				while(!main_queue.empty()){
					mainQ=main_queue.top();
					shortest_path_star(mainQ.a,mainQ.b,mainQ.versionCounter,mainQ.resultsCounter);
					main_queue.pop();
				}
		...
		while(cin >> c) {
			if(c == 'F') {
				...
				while(!thread_queue.empty()){
					thrQ=thread_queue.top();
					query_pool.postWork<void>([thrQ] {  shortest_path_fast(thrQ.a, thrQ.b, thrQ.versionCounter, thrQ.resultsCounter);  });
					thread_queue.pop();
				}
				...
				while(!main_queue.empty()){
					mainQ=main_queue.top();
					shortest_path_fast(mainQ.a,mainQ.b,mainQ.versionCounter,mainQ.resultsCounter);
					main_queue.pop();
				}
